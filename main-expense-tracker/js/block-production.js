  ...
  function renderTrend(trend) {
    ...
    trendChart = new Chart(ctx, {
      ...
      data: {
        labels,
        datasets: [
          { label: 'Blocks Produced', data: trend.map(t => t.blocksProduced), borderColor: '#1656F5', backgroundColor: 'rgba(22,86,245,0.08)', yAxisID: 'y', tension: 0.3, fill: true },
          { label: 'Avg Blocks / Bag', data: trend.map(t => t.avgPerCement), borderColor: '#DD9827', backgroundColor: 'transparent', yAxisID: 'y1', tension: 0.3, borderDash: [4, 3] }
        ]
      },
      options: {
        ...
        scales: {
          y: { beginAtZero: true, position: 'left', title: { display: true, text: 'Blocks / day' } },
          y1: { beginAtZero: true, position: 'right', grid: { drawOnChartArea: false }, title: { display: true, text: 'Blocks / bag' } }
        }
      }
    });
  }
